...
public function listar_cursos_home(){
  ...
  foreach ($datos_curso as $key => $datos_materia) { 
    ...
    <div style="margin-top:5%" class="menu_contextual<?php echo $datos_materia['id_asignacion'] ?>">
        <a class="enlace_sin_estilo" title="<?php echo $datos_materia['descripcion']; ?>" href="<?php echo SGA_CURSOS_URL.'/curso.php?asignacion='.$datos_materia['id_asignacion']; ?>" > <?php echo puntos_suspensivos($datos_materia['nombre_materia'],20).'('.$datos_materia['nombre_categoria_curso'].') <br/>'?></a>
    <?php 
