...
<?php boton_modal_elegir_icono();?>
  ...
  <div class="col-xs-12">
    ...
    <input onclick="tipoinput(this.value)" type="radio" class="" name="adjunto"   id="adjunto" value="si" <?php if($red->adjunto=="si") echo "checked"; ?><?php  ?>>Si</label>
    <label class="checkbox-inline">
    <input onclick="tipoinput(this.value)" type="radio"  class="" name="adjunto" checked  value="no" <?php if($red->adjunto=="no") echo "checked" ;?>>No</label></p>
    </div>
    </div>
...
<?php
echo "<pre>";
print_r(json_decode($red->materia_red));
echo "</pre>";
...
<?php
$academico=new Academico();
($datosmaterias=$academico->consultar_materia($_GET['asignacion']));
?>
...
if(in_array($valor['id_materia'],json_decode($red->materia_red))){
 ...
 if(isset($_GET['asignacion'])){         
  if(($valor['id_materia']==$datosmaterias[0]->id_asignatura)){
    echo "Selected";
   } }
...
if(isset($_GET['asignacion'])){
if($valor['nombre_categoria_curso']==$datosmaterias[0]->nombre_categoria_curso){
    echo "Selected";
}
