...
$datos_curso=$notas->consultar_materia($asignacion)[0];
...
if(isset($_GET['guardar'])){
  ...
  file_put_contents('seguimiento/'.$datos_curso->nombre_materia.'/anotaciones/Anotaciones de '.$datos_curso->nombre_materia.'.pdf', $output);
  echo "<script>window.close();</script>";
 }else{
  $dompdf->stream(' Anotaciones de '.$datos_curso->nombre_materia.".pdf", array("Attachment"=>0));
 }
