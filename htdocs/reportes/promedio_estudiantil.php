...
$inf_ie ="SELECT * FROM `institucion_educativa` WHERE id_institucion_educativa ='1'";
...
$html.='<img style="margin-top:-5%" class="logo" height="60" src="'.SGA_COMUN_SGA_DATA_BANNER.'/'.$rowinfosede["logo_institucion"].'"></img>
...
$nombre_asignatura=($nombre_asignatura[0]->nombre_materia);

echo "<h1 align='center'>ESTADISTICAS DE ".$nombre_asignatura."</h1>";
...
<td>'.formatofechayhora($row['ultima_sesion']).'</td>
 ...
 <?php
  ...
  require_once ("../comun/funciones.php");
  if(empty(consultar_datos2($sql_periodos_con_actividades)))?>
...
foreach(consultar_datos2($sql_periodos_con_actividades) as $array => $informacion){
...
if(isset($inscripcion)){
   ...
   $sqlvaloraciones = 'SELECT * from `inscripcion` inner join usuario on `inscripcion`.id_estudiante = usuario.id_usuario inner join `asignacion` on `inscripcion`.id_asignacion = `asignacion`.id_asignacion inner join `actividad` on `asignacion`.`id_asignacion` = `actividad`.`id_asignacion` left join seguimiento on actividad.id_actividad = seguimiento.id_actividad and `inscripcion`.id_inscripcion = `seguimiento`.id_inscripcion where `fecha_inscripcion` LIKE "%'.$ano.'%"  and actividad.periodo="'.$periodo.'" and  seguimiento.id_inscripcion = '.$inscripcion.' and `asignacion`.id_asignacion="'.$asignaciones.'" order by usuario.apellido  ';
